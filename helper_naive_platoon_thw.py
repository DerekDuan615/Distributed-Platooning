...
def set_platoon_initial_info(L, p, tau, init_vel, delta, n):
    ...
    cav_info = [[_ * -(L + p*tau*init_vel + delta) for _ in range(1, n + 1)], [init_vel for _ in range(1, n + 1)]]
...
class NaivePlatoon:
    ...
    def compute_dual(self):
        ...
        for i in range(1, self.n):
            gap = self.x_ite[self.ite_l-1][self.ite_u][i-1] - self.x_ite[self.ite_l-1][self.ite_u][i]\
                  - (self.L + self.p * self.tau * self.v_ite[self.ite_l-1][self.ite_u][i])
            self.l_ite[self.ite_l][i] = self.l_ite[self.ite_l - 1][i] + 0.01 * min(gap, 0)

    def check_dual_stop(self):    # when gap<0: False;  gap>0: True→Stop
    ...
    def record_trajectory(self, k):
        ...
        for i in range(1, self.n):
            ...
            cur_df[f"fv{i+1}_h"] = (cur_df[f"fv{i+1}_s"]) / cur_df[f"fv{i+1}_Vel"]
