...
class Veh:
    ...
    def __init__(self,
                 ...
                 min_acc=-8,         # m/s^2
                 max_acc=1.35,       # m/s^2
                 gap_l=50,           # constant gap depending on vehicle length (m)
    ...
    def Initialize_Variables(self, lead_traj):
        ...
        self.ego_pos_array[0] = self.lead_pos_array[0] - self.gap_L*self.ID  #
    ...
    def Initialize_Controller(self, tau, q4new, q1, q7new, qz, q4_new, q_1, q7_new, q_z, Vel_buffer, Pos_buffer):

        # Get all vehicles' velocity and position from buffer
        # Define vector a, b and c based on initial positions and velocities of platoon, and acc of precede vehicle
        self.a = np.zeros(self.num_veh)   # row vector
        self.b = np.zeros(self.num_veh)   # row vector

        for i in range(self.num_veh):
            self.b[i] = Vel_buffer[i] - Vel_buffer[i+1]
            self.a[i] = Pos_buffer[i] - Pos_buffer[i+1] - self.gap_L - 2 + tau*(self.b[i])

        self.c = 0.5*(pow(tau,4)/4*self.lead_acc_array[self.idx]*(q4new - q1) + pow(tau,2)*np.dot(self.a, (q7new - qz))
                      + 2*pow(tau,2)*self.lead_acc_array[self.idx]*(q4_new-q_1) + 2*tau*np.dot(self.b, (q7_new-q_z))) # row vector
    ...
    def function_g(self,tau,x1,x2,v1,v2,u1,u2):
        ...
        output_g = -pow(tau,2)*pow(u2, 2)/(2*self.min_acc) +\
                   (self.r*tau+pow(tau,2)/2-(v2-self.min_speed)*tau/(2*self.min_acc))*u2\
    ...
    def function_h(self, v2, tau):
        ...
        output_h = self.r*tau+pow(tau,2)/2-(v2-self.min_speed)*tau/(2*self.min_acc)
