...
class Platoon:
    """platoon object"""
    def __init__(self, n, k, cav_info, hdv_info, columnName, ite_max_u=100, ite_max_lambda=40):
    ...
    def initialize_control_variables(self, k):
        ...
        self.l_ite = np.full((self.ite_max_l, self.n), 0)  # iteration of lambda at next step k+1
    ...
    def mpc_algorithm(self, k):
        ...
        # print(f"dual_idx at {k} time step: {self.dual_idx}")
        self.update_state(k)
    ...
    def calculate_ite_state(self):
        ...
        self.zx_ite[self.ite_l][self.ite_u][0] = self.x_hdv + tau * self.v_hdv + tau**2/2 * self.u_hdv - self.x_ite[self.ite_l][self.ite_u][0] -\
                                                 (L + p*tau*self.v_ite[self.ite_l][self.ite_u][0] + delta)
        for i in range(self.n - 1):
            ...
            self.zx_ite[self.ite_l][self.ite_u][i+1] = self.x_ite[self.ite_l][self.ite_u][i] - \
                                                     self.x_ite[self.ite_l][self.ite_u][i+1] - \
                                                       (L + p*tau*self.v_ite[self.ite_l][self.ite_u][i+1] + delta)

    def compute_gradient(self):
        for i in range(self.n - 1):
            self.gradient_ite[self.ite_l][self.ite_u][i] = -tau**2 * (1/2 + p) * (self.zx_ite[self.ite_l][self.ite_u-1][i]
                                                           *alpha[i]) + tau**2 / 2 * self.zx_ite[self.ite_l][self.ite_u-1][i+1]*alpha[i+1] + \
                                                           ...
                                                           + self.l_ite[self.ite_l][i] * (p * tau ** 2 + tau ** 2 / 2) - \
                                                           self.l_ite[self.ite_l][i + 1] * tau ** 2 / 2
        self.gradient_ite[self.ite_l][self.ite_u][n-1] = -tau**2 * (1/2 + p) * self.zx_ite[self.ite_l][self.ite_u-1][n-1] * alpha[n-1]+\
                                                          tau**2 * self.u_ite[self.ite_l][self.ite_u-1][n-1] - \
                                                         tau * self.zv_ite[self.ite_l][self.ite_u-1][n-1]*beta[n-1] \
                                                         + self.l_ite[self.ite_l][n - 1] * (p * tau ** 2 + tau ** 2 / 2)
    ...
    def compute_dual(self):
        ...
        #     gap = self.x_ite[self.ite_l-1][self.ite_u][i-1] - self.x_ite[self.ite_l-1][self.ite_u][i]\
        #           - (L + p* tau * self.v_ite[self.ite_l-1][self.ite_u][i])
        #     self.l_ite[self.ite_l][i] = self.l_ite[self.ite_l - 1][i] + 0.01 * min(gap, 0)
        for i in range(1, self.n):
            a = 0.05
            b = 0.01
            gap =  L + p * tau * self.v_ite[self.ite_l-1][self.ite_u][i] \
                   - (self.x_ite[self.ite_l-1][self.ite_u][i-1] - self.x_ite[self.ite_l-1][self.ite_u][i])

            if gap > 0:
            # print(f"gap > 0 at iteration {self.ite_l - 1} for vehicle {i}, gap = {gap}")
            # print(f"lambda value before: {self.l_ite[self.ite_l-1][i]}")
                self.l_ite[self.ite_l][i] = self.l_ite[self.ite_l - 1][i] + 1 / (a + b * self.ite_l ) * max(gap, 0)
            # print(f"lambda value after: {self.l_ite[self.ite_l][i]}")
    ...
    def record_trajectory(self, k):
        ...
        cur_df[f"cav1_s"] = self.hdv[0][k] - self.x[0] - L
        cur_df[f"cav1_h"] = cur_df[f"cav1_s"] / cur_df[f"cav1_v"]
        for i in range(1, self.n):
            cur_df[f"cav{i+1}_s"] = self.x[i-1] - self.x[i] - L
            cur_df[f"cav{i+1}_h"] = (cur_df[f"cav{i+1}_s"]) / max(cur_df[f"cav{i+1}_v"], 0.1)
...
def output_to_plot():
    ...
    ax5 = plt.gca()
    for i in range(1, n + 1):
        platoon.trajectory.plot(kind='line', x="time_step", y=f'cav{i}_h', ax=ax5)
    ax5.set_ylabel("headway (sec)")
    ax5.set_xlabel("time (sec)")
    ax5.set_ylim([0.4,0.8])
    ax5.set_title("headway-time plot")
    plt.savefig("headway-time.png")
    plt.show()
...
def set_platoon_initial_info():
    ...
    cav_info = [[_ * -(L + p*tau*init_vel + bias) for _ in range(1, n + 1)], [init_vel for _ in range(1, n + 1)]]
    cav_info[0][-1] = cav_info[0][-2] -(L + p*tau*init_vel -3)
    columnName = ["hdv_x", "hdv_v", "hdv_u"]  # , "iteration_u", "iteration_lambda"
    for i in range(1, n + 1):
        columnName += [f"cav{i}_x", f"cav{i}_s", f"cav{i}_v", f"cav{i}_u", f"cav{i}_h" ,f"cav{i}_u_ite", f"cav{i}_l_ite"]
...
for k in range(0, T-1):
    ...
    comp_time.append(time.process_time() - t0)
    print(time.process_time() - t0)
platoon.trajectory["time_step"] = platoon.trajectory.index/2

"""plot to see performance"""
average_time = sum(comp_time) / len(comp_time)
output_to_plot()